...
long lengthStackRecursive(List list) {
    ...
    if(list == NULL)
    return 0;
    else return 1 + lengthStackRecursive(list->next);
    
}

long tail_recursion(List list , long ac){
    if(list == NULL)
    return ac;
    else return tail_recursion(list->next , ac + 1);
}

long lengthTailRecursive(List list) {
    // TODO : Cerința 3
    return tail_recursion(list,0);
}
